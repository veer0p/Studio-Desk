...
export async function GET(
  ...
  try {
    ...
    const proposal = await ProposalService.getProposalById(supabase, (await props.params).id, member.studio_id)
...
export async function DELETE(
  ...
  try {
    const { member, supabase } = await requireAuth(req)
    await ProposalService.deleteProposal(supabase, (await props.params).id, member.studio_id)

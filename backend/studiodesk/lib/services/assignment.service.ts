...
function briefDbPayload(studioId: string, bookingId: string, data: any) {
  return {
    ...
    venue_access_notes: data.location_notes ?? data.venue_address ?? null,
    contact_on_day: data.client_name ?? null,
    contact_phone: data.client_phone ?? null,
    special_instructions: data.special_requests ?? data.special_instructions ?? null,
...
function briefResponse(row: any) {
  ...
  return {
    ...
    location_notes: row.venue_access_notes ?? null,
    ...
    special_requests: row.special_instructions ?? null,

...
function stringifyMeta(value: any) {
...
async function currentMeta(supabase: Db, galleryId: string) {
  const { data } = await (supabase as any).from('galleries').select('immich_library_id').eq('id', galleryId).maybeSingle()
...
export const galleryRepo = {
  async getGalleries(supabase: Db, studioId: string, params: any) {
    let query = (supabase as any).from('galleries').select(LIST_SELECT, { count: 'exact' }).eq('studio_id', studioId)
  ...
  async getGalleryBySlug(supabase: Db, slug: string) {
    const { data, error } = await (supabase as any).from('galleries').select(DETAIL_SELECT).eq('slug', slug).maybeSingle()
    if (error) throw Errors.validation('Failed to fetch gallery')
    return data ? shapeGallery(data) : null
  },

  async createGallery(supabase: Db, data: any) {
    const meta = data.compat_meta as any | undefined
    const payload = { ...data, compat_meta: undefined, immich_library_id: stringifyMeta(meta ?? {}) }
    const { data: row, error } = await (supabase as any).from('galleries').insert(payload as any).select('id').single()
  ...
  async upsertFaceCluster(supabase: Db, data: any) {
  ...
  async createUploadJob(supabase: Db, data: any) {
    const { data: row, error } = await (supabase as any).from('file_upload_jobs').insert(data as any).select('*').single()
    if (error || !row) throw Errors.validation('Failed to create upload job')
    return row
  },

  async updateUploadJob(supabase: Db, jobId: string, data: any) {
    const { error } = await supabase
      .from('file_upload_jobs')
  ...
  async createGalleryPhoto(supabase: Db, data: any) {
    const { error } = await (supabase as any).from('gallery_photos').insert(data as any)

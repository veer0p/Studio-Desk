...
export const contractRepo = {
  async getContracts(
    ...
    let query = (supabase as any).from('contracts').select(SUMMARY_SELECT, { count: 'exact' }).eq('studio_id', studioId)
  ...
  async createContract(supabase: SupabaseClient<any>, data: any) {
  ...
  async createTemplate(supabase: SupabaseClient<any>, data: any) {
    if (data.is_default) {
      await supabase
        .from('contract_templates')
  ...
  async getDefaultTemplate(supabase: SupabaseClient<any>, studioId: string, eventType: string | null) {
    const { data, error } = await supabase
      ...
      .order('event_type', { ascending: false, nullsFirst: false })
      .order('is_default', { ascending: false })
      .limit(10)

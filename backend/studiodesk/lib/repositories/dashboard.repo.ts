...
async function countAndOldest(
  ...
  let query = (supabase as any).from(table).select(column, { count: 'exact' })
  ...
  return {
    count: count ?? 0,
    oldest: oldestColumn ? isoDate((data?.[0] as any | undefined)?.[oldestColumn]) : isoDate((data?.[0] as any | undefined)?.[column]),
  }
}
...
export const dashboardRepo = {
  ...
  async getTeamPerformanceData(supabase: Db, studioId: string, fromDate: string, toDate: string) {
    const [membersRes, assignmentsRes] = await Promise.all([
      (supabase as any).from('studio_members').select('id, display_name, role').eq('studio_id', studioId).eq('is_active', true),
      supabase
  ...
  async upsertRevenueSnapshot(supabase: Db, studioId: string, snapshotDate: string, payload: any) {

...
function settingsPayload(studioId: string, setting: any, seed = false) {
  const payload: any = {
    studio_id: studioId,
...
export const automationRepo = {
  ...
  async upsertSettings(supabase: Db, studioId: string, settings: Array<any>) {
    const { data, error } = await supabase
      .from('automation_settings')
  ...
  async ensureDefaultSettings(supabase: Db, studioId: string, defaults: Array<any> = DEFAULT_AUTOMATION_SETTINGS as unknown as Array<any>) {
  ...
  async getAutomationLog(supabase: Db, studioId: string, params: any) {
    let query = (supabase as any).from('automation_log').select(LOG_SELECT, { count: 'exact' }).eq('studio_id', studioId)
  ...
  async createLogEntry(supabase: Db, data: any) {
    ...
    const { data: row, error } = await (supabase as any).from('automation_log').insert(payload as any).select(LOG_SELECT).single()
    if (error || !row) throw Errors.validation('Failed to create automation log')
    return shapeLog(row)
  },

  async updateLogStatus(supabase: Db, logId: string, status: string, extraFields: any = {}) {
    const now = nowIso()
    const payload: any = { ...extraFields, status }

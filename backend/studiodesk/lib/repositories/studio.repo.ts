...
export const studioRepo = {
  async getProfileById(supabase: any, studioId: string) {
  ...
  async updateProfile(supabase: any, studioId: string, data: StudioUpdate) {
  ...
  async checkSlugAvailable(supabase: any, slug: string, excludeStudioId: string) {
    const { data, error } = await supabase
      .from('studios')
      .select('id')
      .eq('slug', slug)
  ...
  async getStorageStats(supabase: any, studioId: string) {
    const { data, error } = await supabase
      .from('studios')
  ...
  async getOnboardingStatus(supabase: any, studioId: string) {
    const { data, error } = await supabase
      .from('studios')
  ...
  async getOnboardingEvents(supabase: any, studioId: string) {
    const { data, error } = await supabase
  ...
  async createStudio(supabase: any, data: { name: string; slug: string; email: string }) {
    const { data: row, error } = await supabase
      .from('studios')
      .insert({
        ...data,

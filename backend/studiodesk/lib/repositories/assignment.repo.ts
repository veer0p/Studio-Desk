...
function assignmentStatus(row: any, meta: any) {
...
export const assignmentRepo = {
  ...
  async createAssignment(supabase: Db, data: any) {
  ...
  async updateAssignment(supabase: Db, assignmentId: string, studioId: string, data: any) {
    const { data: row, error } = await supabase
      .from('shoot_assignments')
  ...
  async upsertShootBrief(supabase: Db, data: any) {
    const { data: row, error } = await supabase
      .from('shoot_briefs')

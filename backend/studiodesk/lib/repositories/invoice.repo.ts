...
export const invoiceRepo = {
  async getInvoices(supabase: Db, studioId: string, params: any) {
    let query = (supabase as any).from('invoices').select(LIST_SELECT, { count: 'exact' }).eq('studio_id', studioId)
  ...
  async createInvoice(supabase: Db, data: any) {
  ...
  async createLineItems(supabase: Db, items: Array<any>) {
    const { data, error } = await supabase
      .from('invoice_line_items')
      .insert(items)
      .select('id, invoice_id, studio_id, sort_order, name, description, hsn_sac_code, quantity, unit_price, total_price, created_at')
  ...
  async updateInvoice(supabase: Db, invoiceId: string, studioId: string, data: any) {
    const { data: row, error } = await supabase
      .from('invoices')
  ...
  async updateInvoiceStatus(supabase: Db, invoiceId: string, status: string, extraFields?: any) {
    const { data, error } = await supabase
      .from('invoices')
  ...
  async updateInvoiceRazorpay(supabase: Db, invoiceId: string, data: any) {
  ...
  async getFinancialSummary(supabase: Db, studioId: string, period: 'this_month' | 'last_month' | 'this_quarter' | 'this_fy') {
    ...
    const [{ data: payments }, { data: outstanding }, { data: overdue }, { count: bookings }, { count: leads }] =
      await Promise.all([
        (supabase as any).from('payments').select('amount').eq('studio_id', studioId).eq('status', 'captured').gte('captured_at', fromIso).lte('captured_at', toIso),
        (supabase as any).from('invoices').select('amount_due').eq('studio_id', studioId).in('status', ['sent', 'partially_paid', 'overdue']).gt('amount_due', 0),
        (supabase as any).from('invoices').select('total_amount').eq('studio_id', studioId).eq('status', 'overdue').gt('amount_due', 0),
        (supabase as any).from('bookings').select('id', { count: 'exact', head: true }).eq('studio_id', studioId).gte('event_date', dateOnly(range.start) ?? '').lte('event_date', dateOnly(range.end) ?? ''),
        (supabase as any).from('leads').select('id', { count: 'exact', head: true }).eq('studio_id', studioId).gte('created_at', fromIso).lte('created_at', toIso),
      ])

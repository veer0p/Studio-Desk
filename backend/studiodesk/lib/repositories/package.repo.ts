...
export const packageRepo = {
  async getPackages(supabase: any, studioId: string) {
  ...
  async getNextSortOrder(supabase: any, studioId: string): Promise<number> {
    const { data, error } = await supabase
      .from('service_packages')
      .select('sort_order')
      .eq('studio_id', studioId)
  ...
  async getAddons(supabase: any, studioId: string) {
    const { data, error } = await supabase
      .from('package_addons')

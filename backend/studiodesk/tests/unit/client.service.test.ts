...
describe('ClientService', () => {
  ...
  describe('getClientDetail', () => {
    it('throws NOT_FOUND when client does not exist', async () => {
      ...
      vi.mocked(clientRepo.getClientStats).mockResolvedValue({ total_bookings: 0, total_revenue: 0, total_paid: 0 } as any)
    ...
    it('runs three queries in parallel and returns combined detail', async () => {
      ...
      vi.mocked(clientRepo.getClientStats).mockResolvedValue({ total_bookings: 1, total_revenue: 85000, total_paid: 85000 } as any)

...
beforeEach(() => {
  ...
  mocks.ordersAll.mockResolvedValue({ data: [] } as any)
  mocks.transport.sendWhatsApp.mockResolvedValue({ messageId: 'msg_1' } as any)
  vi.stubGlobal('fetch', vi.fn())
  ;(globalThis.fetch as any).mockResolvedValue({ ok: true, json: async () => ({ major: 1, minor: 2 } as any) })
})

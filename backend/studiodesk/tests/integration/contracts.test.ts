...
describe('Contracts API Integration', () => {
  ...
  it('POST /contracts rejects photographer and missing templates, then creates a valid draft', async () => {
    ...
    await (admin.from('contracts') as any).update({ template_id: null }).eq('studio_id', STUDIO_A_ID)
    await (admin.from('contract_templates') as any).delete().eq('studio_id', STUDIO_A_ID)
    expect(
      (
        await makeRequest('POST', '/api/v1/contracts', {
  ...
  it('POST /contracts/:id/send sends draft, resends sent, and logs activity', async () => {
    const admin = createAdminClient()
    const { count: beforeFeed } = await (admin.from('booking_activity_feed') as any).select('id', { head: true, count: 'exact' }).eq('studio_id', STUDIO_A_ID).eq('event_type', 'contract_sent')
    const { count: beforeAutomation } = await (admin.from('automation_log') as any).select('id', { head: true, count: 'exact' }).eq('studio_id', STUDIO_A_ID).eq('channel', 'email')
    ...
    const beforeResend = await (admin.from('contracts') as any).select('sent_at').eq('id', CONTRACT_SENT_ID).single()
    const resend = await makeRequest('POST', `/api/v1/contracts/${CONTRACT_SENT_ID}/send`, { token: owner.access_token })
    const afterResend = await (admin.from('contracts') as any).select('sent_at').eq('id', CONTRACT_SENT_ID).single()
    ...
    const { count: afterFeed } = await (admin.from('booking_activity_feed') as any).select('id', { head: true, count: 'exact' }).eq('studio_id', STUDIO_A_ID).eq('event_type', 'contract_sent')
    const { count: afterAutomation } = await (admin.from('automation_log') as any).select('id', { head: true, count: 'exact' }).eq('studio_id', STUDIO_A_ID).eq('channel', 'email')
  ...
  it('POST /contracts/sign/:token validates, signs, updates booking, and logs activity', async () => {
    ...
    expect(res.status).toBe(200)
    const admin = createAdminClient()
    const signed = await (admin.from('contracts') as any).select('status, signed_at, signed_ip').eq('id', CONTRACT_SENT_ID).single()
    const booking = await (admin.from('bookings') as any).select('status').eq('id', BOOKING_CONVERTED_ID).single()
    const activity = await (admin.from('booking_activity_feed') as any).select('id').eq('studio_id', STUDIO_A_ID).eq('booking_id', BOOKING_CONVERTED_ID).eq('event_type', 'contract_signed')

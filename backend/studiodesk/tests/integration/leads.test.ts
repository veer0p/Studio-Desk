...
async function waitForActivity(bookingId: string, maxMs = 3000) {
  ...
  while (Date.now() < deadline) {
    const { data } = await (admin.from('booking_activity_feed') as any)
      .select('id')
...
describe('DELETE /api/v1/leads/:id', () => {
  ...
  it('204 marks lead as lost (not deleted from DB)', async () => {
    ...
    const { data } = await (admin.from('leads') as any).select('status').eq('id', lid).single()
  ...
  it('lead still exists in DB with status=lost after delete', async () => {
    ...
    const { data } = await (admin.from('leads') as any).select('id,status').eq('id', lid).single()
...
describe('POST /api/v1/leads/:id/convert', () => {
  ...
  it('201 valid convert → booking created in DB', async () => {
    ...
    const { data: b } = await (admin.from('bookings') as any).select('id').eq('id', bid).single()
    expect(b).toBeTruthy()
  })
  ...
  it('201 lead.converted_to_booking = TRUE after convert', async () => {
    ...
    expect(status).toBe(201)
    const bid = (body as { data: any }).data.booking_id
    const { data: ld } = await (admin.from('leads') as any).select('converted_to_booking').eq('id', lid).single()
    expect((ld as { converted_to_booking: boolean }).converted_to_booking).toBe(true)
    await (admin.from('bookings') as any).delete().eq('id', bid)
    await (admin.from('leads') as any).delete().eq('id', lid)
  })
  ...
  it('201 lead.status = contract_signed after convert', async () => {
    ...
    expect(status).toBe(201)
    const bid = (body as { data: any }).data.booking_id
    const { data: ld } = await (admin.from('leads') as any).select('status').eq('id', lid).single()

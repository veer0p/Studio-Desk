...
export async function resetAssignmentFixtures() {
  ...
  await (admin.from('booking_activity_feed') as any).delete().eq('studio_id', STUDIO_A_ID).in('event_type', ['team_assigned', 'shoot_confirmed', 'status_changed', 'note_added'])
  await (admin.from('automation_log') as any).delete().eq('studio_id', STUDIO_A_ID).eq('automation_type', 'custom')
  await (admin.from('shoot_assignments') as any).delete().eq('studio_id', STUDIO_A_ID)
  await (admin.from('shoot_briefs') as any).delete().eq('studio_id', STUDIO_A_ID)
  await (admin.from('member_unavailability') as any).delete().eq('studio_id', STUDIO_A_ID)

  await (admin.from('studio_members') as any).upsert(
    [
      {
  ...
  await (admin.from('shoot_assignments') as any).insert([
    {
  ...
  await (admin.from('member_unavailability') as any).upsert(
    {

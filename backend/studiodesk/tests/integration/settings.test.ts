...
async function seedSettingsFixtures() {
  ...
  await (admin.from('studios') as any).update({
  ...
  await (admin.from('studio_settings') as any).upsert(
    [
      {
...
describe('Settings API Integration', () => {
  ...
  it('GET /settings/notifications falls back to defaults when the row is missing', async () => {
    const admin = createAdminClient()
    await (admin.from('studio_settings') as any).delete().eq('studio_id', STUDIO_A_ID)
  ...
  it('stores Studio A integrations encrypted at rest in the database', async () => {
    const admin = createAdminClient()
    const { data, error } = await (admin.from('studios') as any)

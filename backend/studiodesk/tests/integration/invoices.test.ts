...
describe('Invoices API Integration', () => {
  ...
  it('GET /invoices handles auth, filters, pagination, and auto overdue marking', async () => {
    ...
    await (admin.from('invoices') as any).update({ status: 'sent', due_date: new Date(Date.now() - 86400000).toISOString().slice(0, 10) }).eq('id', INVOICE_SENT_ID)
    ...
    for (let i = 0; i < 10; i += 1) {
      const overdue = await (admin.from('invoices') as any).select('status').eq('id', INVOICE_SENT_ID).single()

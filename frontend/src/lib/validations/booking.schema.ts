...
export const createBookingSchema = z.object({
  ...
  event_date: z.preprocess((v) => (v === '' ? undefined : v), z.string().refine((v) => !isNaN(Date.parse(v)), { message: 'Invalid date' }).optional().nullable()),
  event_end_date: z.preprocess((v) => (v === '' ? undefined : v), z.string().refine((v) => !isNaN(Date.parse(v)), { message: 'Invalid date' }).optional().nullable()),

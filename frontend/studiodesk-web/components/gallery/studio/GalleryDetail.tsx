...
export function GalleryDetail({ galleryId }: { galleryId: string }) {
  ...
  const handleShare = () => {
    const url = `https://gallery.studiodesk.com/${gallery.slug}`
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className={`px-2 py-0.5 rounded-sm text-[9px] font-mono font-bold tracking-widest uppercase border ${statusConfig[gallery.status].color}`}>
            ...
            >
              gallery.studiodesk.com/{gallery.slug} <ExternalLink className="w-3 h-3 ml-1" />
            </a>
          </div>
              ...
              {steps.map((step, idx) => {
                ...
                return (
                  <div key={step.id} className="flex flex-col items-center gap-2 -ml-2 first:ml-0 -mr-2 last:mr-0 z-10 w-24">
                    ...
                    <span className={`text-[8px] font-mono tracking-widest uppercase font-bold text-center ${
            ...
            <div className="flex-1 h-full overflow-y-auto custom-scrollbar flex flex-col relative">
              <div className="p-8 pb-32">
                <div className="w-full h-48 border-2 border-dashed border-border/60 rounded-md flex items-center justify-center text-muted-foreground flex-col gap-2">
                  <p className="font-mono text-[10px] tracking-widest uppercase font-bold">Photos Grid Offline</p>
                  <p className="text-[9px] font-mono tracking-widest uppercase">Drag files to the right panel instead</p>
                </div>
              </div>

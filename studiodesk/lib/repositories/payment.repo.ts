...
export const paymentRepo = {
  ...
  async createPayment(supabase: Db, data: Record<string, unknown>) {
    const { data: row, error } = await supabase.from('payments').insert(data).select('*').single()
    if (error) throw Errors.validation('Failed to create payment')
    return flatten(row)
  },

  async updatePaymentStatus(supabase: Db, paymentId: string, status: string, extraFields?: Record<string, unknown>) {
  ...
  async createRefund(supabase: Db, data: Record<string, unknown>) {
    const { data: row, error } = await supabase.from('refunds').insert(data).select('*').single()
    if (error) throw Errors.validation('Failed to create refund')
    return row
  },

  async logWebhook(supabase: Db, data: Record<string, unknown>) {
    const insert = async (payload: Record<string, unknown>) => {
      const { data: row, error } = await supabase.from('webhook_logs').insert(payload).select('id').single()
  ...
  async updateWebhookLog(supabase: Db, logId: string | null, data: Record<string, unknown>) {
    if (!logId) return
    try {
      await supabase.from('webhook_logs').update(data).eq('id', logId)
    } catch {}
  },
  ...
  async logPaymentGateway(supabase: Db, data: Record<string, unknown>) {
    try {
      await supabase.from('payment_gateway_logs').insert(data)
    } catch {}
  },

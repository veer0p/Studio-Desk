...
async function resetTeamInviteFixtures(admin: ReturnType<typeof createClient>): Promise<void> {
  ...
  const { error: cleanupStudioAMembersError } = await admin
    .from('studio_members')
    .delete()
  ...
  const { error: cleanupStudioBMembersError } = await admin
    .from('studio_members')
    .delete()
  ...
  const { error: cleanupInvitesError } = await admin
  ...
  const { error: resetInvitesError } = await admin
    .from('studio_invitations')
    .upsert(
      [
        {
...
async function resetLeadFixtures(admin: ReturnType<typeof createClient>): Promise<void> {
  ...
  const { data, error } = await admin.from('leads').select('id, studio_id')
...
export default async function globalSetup() {
  ...
  const { error: rlErr } = await admin.from('rate_limits').delete().like('key', 'team_accept:%')
  if (rlErr) {
    console.error('[integration global-setup] rate_limits cleanup:', rlErr.message)
  }

  const now = new Date().toISOString()
  const { error: onboardErr } = await admin.from('studio_onboarding_events').upsert(
    [
      {

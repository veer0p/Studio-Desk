...
describe('POST /api/v1/inquiry', () => {
  ...
  beforeAll(async () => {
    ...
    await admin.from('rate_limits').delete().like('key', 'inquiry:%')
  })
  ...
  it('201 valid submission → lead created in DB', async () => {
    ...
    const admin = createAdminClient()
    const { data } = await admin.from('leads').select('id').eq('id', leadId).single()
    expect(data).toBeTruthy()
  })

  it('201 same phone as existing client → reuses client', async () => {
    const admin = createAdminClient()
    const { count: before } = await admin.from('clients').select('id', { count: 'exact', head: true }).eq('studio_id', STUDIO_A_ID)
    const { status } = await makeRequest('POST', '/api/v1/inquiry', {
      query: { studio: STUDIO_SLUG },
      body: { full_name: 'Reuse Test', phone: '9876543210' },
    })
    expect(status).toBe(201)
    const { count: after } = await admin.from('clients').select('id', { count: 'exact', head: true }).eq('studio_id', STUDIO_A_ID)
  ...
  it('form_data preserved in leads table', async () => {
    ...
    const leadId = (body as { data: any }).data.lead_id
    const admin = createAdminClient()
    const { data } = await admin.from('leads').select('form_data').eq('id', leadId).single()

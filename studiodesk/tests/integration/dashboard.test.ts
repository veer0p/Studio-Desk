...
async function resetDashboardFixtures() {
  ...
  const { data: studioClients } = await admin.from('clients').select('id').eq('studio_id', STUDIO_A_ID).limit(2)
  const clientAId = studioClients?.[0]?.id ?? CLIENT_PRIYA_ID
  const clientBId = studioClients?.[1]?.id ?? studioClients?.[0]?.id ?? CLIENT_MEERA_ID

  await admin.from('revenue_snapshots').delete().eq('studio_id', STUDIO_A_ID)
  await admin.from('revenue_snapshots').insert(buildRevenueSnapshots())

  await admin.from('shoot_assignments').delete().in('id', [TODAY_ASSIGNMENT_A_ID, TODAY_ASSIGNMENT_B_ID])
  await admin.from('shoot_briefs').delete().eq('id', TODAY_BRIEF_ID)
  await admin.from('bookings').delete().in('id', [TODAY_BOOKING_A_ID, TODAY_BOOKING_B_ID])
  ...
  await admin.from('shoot_assignments').upsert(
    [
      {
...
describe('Dashboard API Integration', () => {
  ...
  it('POST /cron/snapshot requires cron secret and upserts today snapshots', async () => {
    ...
    const admin = createAdminClient()
    const activeStudios = await admin.from('studios').select('id').eq('is_active', true).eq('subscription_status', 'active')
    const todayRows = await admin.from('revenue_snapshots').select('studio_id').eq('snapshot_date', ymd(new Date()))

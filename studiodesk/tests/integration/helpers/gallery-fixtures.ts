...
export async function resetGalleryFixtures() {
  ...
  await admin.from('gallery_share_logs').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID])
  await admin.from('gallery_photos').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID])
  await admin.from('gallery_videos').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID])
  await admin.from('face_clusters').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID])
  await admin.from('file_upload_jobs').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID])
  await admin.from('galleries').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID])
  await admin.from('rate_limits').delete().like('key', 'gallery_lookup:%')
  await admin.from('rate_limits').delete().like('key', 'public_gallery:%')
  ...
  await admin.from('galleries').upsert(
    [
      {
        id: GALLERY_DRAFT_ID,
        studio_id: STUDIO_A_ID,
  ...
  await admin.from('face_clusters').insert([
    {
  ...
  await admin.from('file_upload_jobs').insert([
    {

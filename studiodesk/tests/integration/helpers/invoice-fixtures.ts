...
export async function resetInvoiceFixtures() {
  ...
  await admin.from('payments').delete().eq('studio_id', STUDIO_A_ID)
  await admin.from('refunds').delete().eq('studio_id', STUDIO_A_ID)
  await admin.from('invoice_line_items').delete().eq('studio_id', STUDIO_A_ID)
  await admin.from('invoices').delete().eq('studio_id', STUDIO_A_ID)
  await admin.from('payment_gateway_logs').delete().eq('studio_id', STUDIO_A_ID)
  await admin.from('webhook_logs').delete().eq('provider', 'razorpay')
  await admin.from('automation_log').delete().eq('studio_id', STUDIO_A_ID).in('automation_type', ['advance_payment_reminder', 'balance_payment_reminder'])
  await admin.from('email_delivery_logs').delete().eq('studio_id', STUDIO_A_ID)
  await admin.from('booking_activity_feed').delete().eq('studio_id', STUDIO_A_ID).in('event_type', ['advance_invoice_sent', 'advance_payment_received', 'balance_invoice_sent', 'balance_payment_received'])
  await admin
    .from('studios')

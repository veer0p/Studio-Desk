...
export async function resetAutomationFixtures() {
  ...
  const ops = [
    admin.from('automation_log').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID]),
    admin.from('automation_settings').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID]),
    admin.from('whatsapp_templates').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID]),
    admin.from('email_templates').delete().in('studio_id', [STUDIO_A_ID, STUDIO_B_ID]),
    admin.from('rate_limits').delete().like('key', 'automation:%'),
    admin.from('rate_limits').delete().like('key', 'automation_test:%'),
  ]
  for (const op of ops) {
  ...
  const { error: waError } = await admin.from('whatsapp_templates').insert(
    AUTOMATION_WHATSAPP_TEMPLATES.map((row) => ({ studio_id: STUDIO_A_ID, ...row }))
  )
  if (waError) throw waError

  const { error: emailError } = await admin.from('email_templates').insert([
    {
      studio_id: STUDIO_A_ID,
      automation_type: 'gallery_ready',
      name: 'Gallery Ready',
  ...
  const { error: logError } = await admin.from('automation_log').insert(automationLogs())

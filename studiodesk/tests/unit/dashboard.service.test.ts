...
describe('DashboardService', () => {
  ...
  it('computes revenue analytics and handles invalid month inputs', async () => {
    ...
    vi.mocked(dashboardRepo.getRevenueChartData).mockResolvedValue([
      { month: 'Jan 2026', collected: 100000, pending: 20000, overdue: 0 },
      { month: 'Feb 2026', collected: 120000, pending: 10000, overdue: 5000 },
    ])
    ...
    expect(result.chart_data[0]).toMatchObject({ month: 'Jan 2026', collected: 100000, pending: 20000 })
  })

  it('computes booking analytics with zero-safe conversion and full funnel keys', async () => {
    vi.mocked(dashboardRepo.getBookingFunnelData).mockResolvedValue({ proposal_sent: 5, advance_paid: 2 })

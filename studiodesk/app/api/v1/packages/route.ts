...
import { createPackageSchema } from '@/lib/validations/package.schema'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const packages = await PackageService.getPackages(supabase, member.studio_id)
    const res = Response.ok(packages)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const pkg = await PackageService.createPackage(supabase, member.studio_id, parsed.data)
    const res = Response.created(pkg)
    res.headers.set('Cache-Control', 'no-store')
    return res
  } catch (err: unknown) {
    if (err instanceof ServiceError) {

...
import { creditNoteSchema } from '@/lib/validations/invoice.schema'

export async function POST(req: NextRequest, context: any) {
  try {
    const { member, supabase, user } = await requireOwner(req)
    const id = await resolveUuidParam(context, req, 'invoice id')
    const parsed = creditNoteSchema.safeParse(await parseJson(req))
    if (!parsed.success) return Response.error(validationMessage(parsed.error), 'VALIDATION_ERROR', 400)
    return withNoStore(Response.created(await InvoiceService.createCreditNote(supabase, id, member.studio_id, parsed.data, user.id)))
  } catch (err) {

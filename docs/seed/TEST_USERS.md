# Test Users & Super Admin Seed — Documentation

## Overview

This document covers the production-grade seed mechanism for creating test users and the super admin in the StudioDesk application.

## Super Admin

| Email | Password | Role | Auth UUID |
|-------|----------|------|-----------|
| admin@studiodesk.in | Admin@1234 | super_admin | `d0000001-0001-4000-8000-000000000001` |

**Seed command**: `cd backend/studiodesk && npm run db:seed:admin` (or run `npx ts-node -r tsconfig-paths-register --project supabase/tsconfig.seed.json supabase/seed-super-admin.ts`)
